...
public class MemberHandler {
  ...
  static void printMembers() {
    ...
    for (int i = 0; i < count; i++) {
      Member m = members[i];
      System.out.printf("%d\t%s\t%s\t%s\t%s\n", m.no, m.name, m.tel, m.working ? "예" : "아니오", levelText(m));
      System.out.println("---------------------------------------");
    }
  }
  ...
  static void printMember() {
    ...
    System.out.printf("전공여부: %s\n", levelText(m));
    System.out.printf("가입일: %s\n", m.createdDate);
    return;
  }


  static String levelText(Member m) {
    switch (m.level) {
      case 0: return "비전공자";
      case 1: return  "준전공자";
      default: return "전공자";
  ...
  static void modifyMember() {
    ...
    m.name = Prompt.inputString(String.format("이름(%s)? ", m.name));
    m.tel = Prompt.inputString(String.format("전화번호(%s)? ", old.tel));
